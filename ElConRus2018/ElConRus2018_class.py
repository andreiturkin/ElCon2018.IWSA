...
class SimpleRobot(CoveringTree, PlottingTree, Phi):
############################################################################################
# Public Methods
############################################################################################
    def __init__(self, idelta, rhomin, rhomax, thetamin, thetamax, ShowCovPrc=False):
        ...
        Phi.__init__(self, rhomin, rhomax, thetamin, thetamax)

    def SaveResults(self, fileName, AddRings):
        super(SimpleRobot, self).saveResultAsImage(self.getTree()['Tree'], fileName, AddRings=AddRings)

    def SaveSolution(self, fName):
        super(SimpleRobot, self).SaveSolution(fName)

    def isFileExist(self, fName):
        return super(SimpleRobot, self).isFileExist(fName)

    def LoadSolution(self, fName):
        super(SimpleRobot, self).LoadSolution(fName)
